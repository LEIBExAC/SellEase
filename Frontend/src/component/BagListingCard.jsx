...
export default function BagListingCard({ item }) {
  ...
  const handleRemoveCard = async (product_id) => {
    let response = await axios.delete(
      `http://localhost:8080/user/bag/${product_id}`,
      {
        headers: {
  ...
  return (
    <div className="flex items-center justify-between border-b pb-4 mb-4">
      ...
      <div className="flex items-center space-x-2">
        <button className="bg-gray-200 text-gray-600 px-2 rounded-md">-</button>
        <span className="font-medium">{item.quantity}</span>
        <button className="bg-gray-200 text-gray-600 px-2 rounded-md">+</button>
      </div>
      <div className="flex items-start justify-end h-[100px] w-[50px] p-0 m-0 ">
        <ImCross
          onClick={() => handleRemoveCard(item._id)}
          className="text-[2rem] text-[red] font-[300] cursor-pointer hover:bg-slate-200 p-2 rounded-[50%]"
        />{" "}
      </div>
    </div>
  );
